...
#'
#' @examples
Tapply_f <- function(data, var_quant, var_cat, digits = 2){
...
list_tapply_f <- function(data, vars_numeriche, var_cat, digits = 2){
lista_tapply <- list()
k_tap = 1
for (i in vars_numeriche) {
  lista_tapply[[k_tap]] <- Tapply_f(data, var_quant = i, var_cat = var_cat)
...
#prova <- Tapply_f(ana01, var_quant = "NLR", var_cat = "Histology")
#prova <- list_tapply_f(ana01, vars_numeriche = class_df$name_col[class_df$class == "numeric"], var_cat = "Histology")
