...
Lineplots_LB <- function (data, variables, time, breaks = unique(data[, time]),
                          label = unique(data[, time]), group = 1, col_lines = c("salmon",
                                                                                 "royalblue"), stat_line = "median", smooth_line = FALSE,
                          span_line = 0.3, lw_reg = 1, alpha_line = 1, ylim = c(0.2,
                                                                                0.8), ribbon = TRUE, alpha_ribbon = 0.05, ID_lines = FALSE,
                          ...
                          pptx_height = 5.5, target = paste0(path, "/file.pptx"), label_title = paste0("Lineplots by ",
                                                                                                       group, "\n", format(Sys.Date(), "%d/%m/%Y")), size_label_title = 2.5,
                          extra = F, extra_text = NULL)
{
  ...
  themePPTX <- theme(axis.text.x = element_text(size = size_axis_x,
                                                colour = "black", vjust = -0), plot.margin = margin(2,
                                                                                                    2, 2, 2, "mm"), axis.text.y = element_text(size = size_axis_y,
                                                                                                                                               colour = "black"), panel.border = element_rect(linetype = "solid",
                                                                                                                                                                                              colour = "black", linewidth = 0.1, fill = NA), axis.ticks = element_line(linewidth = 0.1),
  ...
  if (length(variables) > 1) {
    list_reg[[1]] <- ggplot(data, aes_string(x = 1, y = 1)) +
      ...
      } + theme_transparent() + annotate(geom = "text", x = 1,
                                         y = 1.001, size = size_label_title, label = label_title,
                                         vjust = 1.25, fontface = "bold") + guides(color = guide_legend(override.aes = list(size = 7.5,
                                                                                                                            shape = 20))) + theme(legend.position = "inside",
  ...
  for (i in variables) {
    ...
    if (Posthoc == T) {
      posthoc_df <- Posthoc_lineplots_LB(Test_results = Test_results,
                                         data = data, time = time, threshold_posthoc = threshold_posthoc,
                                         i)
    }
    ...
    if (stat_line == "median") {
      if (smooth_line) {
        gg <- gg + stat_smooth(data = smooth_data, aes(x = Time_num, y = smooth_y), geom = "line", alpha = alpha_line,
                               size = lw_reg, span = span_line, show.legend = F)
      }
      else {
      ...
      if(smooth_line){
        if(group == 1){
          gg <- gg + stat_smooth(data = smooth_data, aes(x = Time_num, y = Upper), geom = "line", alpha = alpha_line,
                                 size = lw_reg, span = span_line, show.legend = F, colour = NA)
          ...
          gg <- gg + geom_ribbon(data = df2, aes(x = x, ymin = ymin, ymax = ymax, y = 0), fill = "grey", alpha = 0.4, show.legend = F,
                                 linewidth = NA)
        }
        else if(group != 1){
          ...
          # Therapy = ifelse(gg1$data[[3]]$group == 1, "IMMUNO + LT", "LT"
    ...
    gg <- gg + labs(title = i, x = NULL, y = NULL) + scale_x_continuous(breaks = breaks,
                                                                        labels = label, guide = guide_axis(check.overlap = TRUE))
    {
      if (Overall)
        gg <- gg + annotate(geom = "text", x = -Inf,
                            y = Inf, hjust = -0.1, vjust = 1.5, size = posthoc_test_size,
                            label = LandS::formatz_p(Test_results[Test_results[,
                                                                               1] == i, 2]), colour = "black")
    }
    {
      if (Posthoc)
        if (nrow(posthoc_df) > 0 & !all(is.na(posthoc_df)))
          gg <- gg + ggpubr::stat_pvalue_manual(posthoc_df,
                                                label = "p = {pval}", y.position = max(tapply(data[,
                                                                                                   i], data[, time], median, na.rm = T)),
