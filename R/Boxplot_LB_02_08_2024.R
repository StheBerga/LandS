...
#' @param bracket_shorten [0,1] width of the bracket
#' @param bracket.nudge.y Vertical adjustment to nudge brackets by. Useful to move up or move down the bracket. If positive value, brackets will be moved up; if negative value, brackets are moved down.
...
Boxplot_LB <- function (data, variables, group, ID_lines = FALSE, Posthoc = FALSE,
                        ...
                        width_box = 0.2, size_median_line = 0.8, col_median_line = "red",
                        lwd_box = 0.1, lwd_ID_line = 0.2,
                        alpha_ID_line = 0.3, alpha_point = 0.3, size_point = 0.3,
                        Test_results = NULL, threshold_posthoc = 0.1, posthoc_test_size = 3.88,
                        bracket_shorten = 0, bracket.nudge.y = 0,
  ...
  for (i in variables) {
    ...
    if (length(variables) > 1){
      boxplot[[k + 1]] <-
        ...
        ggplot(data = data, aes_string(x = data[, group])) +
        ...
        scale_x_discrete(breaks = breaks_axis_x, labels = labels_axis_x) +
        {
          ...
          if (grid)
            themegrid + theme(plot.title = element_text(hjust = 0.5, size = size_title, vjust = -0.5, face = "bold", colour = col_title))
        } + {
          if (PPTX)
            theme_PPTX + theme(plot.title = element_text(hjust = 0.5, size = 20, vjust = -0.5, face = "bold", colour = col_title))
        }
      ...
      boxplot[[k]] <-
        ...
        {
          if (Point)
            geom_point(aes_string(y = data[, "Y"]), size = size_point, alpha = alpha_point)
        } + {
          if (ID_lines)
            geom_line(aes_string(y = data[, i], group = ID), alpha = alpha_ID_line, linewidth = lwd_ID_line, colour = "black")
        } + {
          if (Median_line)
            stat_summary(aes_string(y = data[, i], group = 1), geom = "line", fun = median, linewidth = size_median_line,
                         colour = col_median_line, linetype = "solid", show.legend = F)
        } + {
          if (rm.outliers)
            coord_cartesian(ylim = c(min(data[, "Y"], na.rm = T),
        ...
        scale_x_discrete(breaks = breaks_axis_x, labels = labels_axis_x) +
        {
          ...
          if (Overall)
            annotate(geom = "text", x = -Inf, y = Inf, hjust = -0.1,
                     vjust = 1.5, size = posthoc_test_size,
